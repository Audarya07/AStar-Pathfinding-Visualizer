...
class Spot:
	...
	def update_neighbours(self, grid):
		self.neighbours = []
		if self.row < self.total_rows - 1 and not grid[self.row + 1][self.col].is_barrier():		#MOVE DOWN is valid
			self.neighbours.append(grid[self.row + 1][self.col])
		
		if self.row > 0 and not grid[self.row - 1][self.col].is_barrier():		#MOVE UP is valid
			self.neighbours.append(grid[self.row - 1][self.col])
		
		if self.col < self.total_rows - 1 and not grid[self.row][self.col - 1].is_barrier():		#MOVE LEFT is valid
...
def algorithm(draw, grid, start, end):
	...
	open_set.put((0, count, start))	#Arguments in put() --> (f_score, count, start)
	came_from = {}	#To trace route
	...
	open_set_hash = {start}		#set to check if an element is present in open_set or not
	...
	while not open_set.empty():
		...
		current = open_set.get()[2]		#get start element from open set
		...
		if current != start:
			current.make_closed()		#Set it to RED so that it's not visited again
